    ...
    if pdb_id in lp_pdbbind['PDB_ID'].values:
        ...
        if subset in ['train', 'val', 'test']:
            ...
            if not pre_computed_data:
                data = graph_featurizer.featurize_complex(construct, complx.pocket.mol, center_pos)
...
def episode(seed_idxs, 
    ...
    current_data = [input_data_list[seed_i] for seed_i in seed_idxs]
    ...
    while step_i < n_steps and not all(next_terminated):
        ...
        batch = Batch.from_data_list(current_data)
        # batch = batch.to(device)
        current_obs = batch.to(device)
