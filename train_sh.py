...
from ymir.sh_env import (FragmentBuilderEnv, 
                      BatchEnv, 
                      Action)
...
from ymir.sh_policy import Agent
from torch.optim import Adam
from torch_geometric.data import Batch, Data
from ymir.molecule_builder import potential_reactions
from e3nn import o3
from ymir.atom_environment import AtomicNumberTable
...
logging.basicConfig(filename='train_sh.log', 
                    encoding='utf-8', 
                    level=logging.INFO,
...
batch_size = min(n_envs, 64) # NN batch, input is Data, output are actions + predicted reward
...
experiment_name = f"ymir_sh_{timestamp}_lr{lr}_ent{ent_coef}"
...
#     for fragment in protected_fragments:
#         fragment.unprotect()
#         n_attach = len(fragment.get_attach_points())
#         n_attaches.append(n_attach)
...
agent = Agent(fragments=final_fragments, 
              z_table=z_table)
...
try:
    ...
    for episode_i in tqdm(range(n_episodes)):
        ...
        obs: list[torch.Tensor] = [] # (n_non_term_envs)
        ...
        while step_i < n_steps and not all(next_terminated):
            ...
            with torch.no_grad():
                ...
                features = agent.extract_features(current_obs)
        ...
        for epoch_i in range(n_epochs):
            ...
            for start in range(0, n_obs, batch_size):
                ...
                features = agent.extract_features(mb_obs)
