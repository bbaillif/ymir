...
from ymir.mace2_env import (FragmentBuilderEnv, 
                      BatchEnv, 
                      Action)
...
from ymir.mace2_policy import Agent
...
logging.basicConfig(filename='train_o3_v2.log', 
                    encoding='utf-8', 
                    level=logging.INFO,
...
batch_size = min(n_envs, 8) # NN batch, input is Data, output are actions + predicted reward
...
experiment_name = f"ymir_mace2_{timestamp}"
    ...
    for episode_i in tqdm(range(n_episodes)):
        ...
        while step_i < n_steps and not all(next_terminated):
            ...
            with torch.no_grad():
                ...
                x.to(device)
                embeddings = agent.extract_embeddings(x)
                current_masks = current_masks.to(device)
                
                current_action, current_logprob, current_entropy = agent.get_action(embeddings, 
                                                                                    masks=current_masks)
                current_values = agent.get_value(embeddings)
        ...
        for epoch_i in range(n_epochs):
            ...
            for start in range(0, n_obs, batch_size):
                ...
                _, current_logprob, entropy = agent.get_action(embeddings,
                                                                masks=current_masks,
