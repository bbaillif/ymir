...
from ymir.atom_environment import SHEnvironment
...
class FragmentBuilderEnv():
    ...
    def __init__(self,
        ...
        self.device = device
        self.sh_environment = SHEnvironment(z_table=self.z_table,
                                            device=device)
    ...
    def featurize_pocket(self) -> torch.Tensor:
        
        complx_mol = Chem.CombineMols(self.pocket_mol, self.seed)
        for atom in complx_mol.GetAtoms():
            atomic_num = atom.GetAtomicNum()
            if atomic_num == 0:
                atom_id = atom.GetIdx()
                break
        atom_environment = self.sh_environment.get_environment(complx_mol,
                                                               atom_id)
    ...
    def get_valid_action_mask(self):
        ...
        # n_clashes = self.get_clashes() # Takes 400 it/s
        # valid_positions = torch.tensor(n_clashes)
        # valid_positions = torch.logical_not(valid_positions)
...
class BatchEnv():
    ...
    def get_obs(self,
                all_envs: bool = False) -> torch.Tensor:
        obs_list = []
        if all_envs:
