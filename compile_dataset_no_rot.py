...
fragments_neighbors = [get_neighbor_symbol(fragment) 
                        for fragment in protected_fragments]
...
with Chem.SDWriter('/home/bb596/hdd/ymir/protected_fragments.sdf') as writer:
    for fragment in protected_fragments:
        ...
        for seed_i, complx_i_seed in enumerate(seed_to_complex):
            if complx_i_seed == complx_i:
                ...
                added_fragments = [Fragment(fragment)
                                    for fragment, valid in zip(protected_fragments, valid_action_mask)
                                    if valid]
                ...
                bond_tuples = []
                for fragment_i, fragment in enumerate(tqdm(added_fragments[:n_fragments])):
